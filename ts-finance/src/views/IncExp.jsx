...
const CAT_COLORS = {
  ...
  'Honorarios':               { bg: 'rgba(124,92,252,.09)',   c: '#5A3EE7'  },
  'Proveedores':              { bg: 'rgba(217,119,6,.09)',   c: '#B45309'  },
  'Arriendo':                 { bg: 'rgba(236,72,153,.09)',  c: '#BE185D'  },
  'Servicios':                { bg: 'rgba(30,158,90,.09)',  c: '#38955A'  },
  'Internet':                 { bg: 'rgba(37,99,235,.09)',   c: '#2563EB'  },
  'Software y suscripciones': { bg: 'rgba(124,92,252,.09)',   c: '#5A3EE7'  },
  'Publicidad':               { bg: 'rgba(239,68,68,.09)',   c: '#DC2626'  },
  'Producción':               { bg: 'rgba(217,119,6,.09)',   c: '#D97706'  },
  'Casino':                   { bg: 'rgba(229,72,77,.09)',   c: '#D72B20'  },
  'Transporte':               { bg: 'rgba(20,184,166,.09)',  c: '#0F766E'  },
  'Imprevistos':              { bg: 'rgba(245,158,11,.09)',  c: '#92400E'  },
  'Impuestos':                { bg: 'rgba(229,72,77,.09)',   c: '#B52319'  },
  'Otros':                    { bg: T.surf2,                 c: T.text2    },
}

/* ── shared primitives ───────────────────────────────────── */
const thCell = {
  textAlign: 'left', fontSize: 11, fontWeight: 700, color: T.subtle,
  padding: '0 14px', textTransform: 'uppercase', letterSpacing: '.06em',
  borderBottom: `2px solid ${T.border}`, whiteSpace: 'nowrap',
  background: 'var(--surf2)', position: 'sticky', top: 0, zIndex: 1, height: 44,
}
const thRight = { ...thCell, textAlign: 'right' }
/* rows 48px height */
const tdBase = { padding: '0 14px', fontSize: 13, color: T.text, whiteSpace: 'nowrap', height: 48 }
...
function ChipBtn({ active, onClick, children }) {
  return (
    <button onClick={onClick} style={{
      ...
      border: active ? `1px solid #062F28` : `1px solid ${T.border}`,
      fontSize: 11, fontWeight: active ? 700 : 500, cursor: 'pointer', whiteSpace: 'nowrap',
      background: active ? '#062F28' : 'var(--surf)',
...
function KpiStrip({ items }) {
  return (
    <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
      {items.map((k, i) => (
        <div key={i} style={{
          flex: '1 1 160px', background: 'var(--surf)', border: '1px solid var(--border)',
          ...
          <div style={{ fontSize: 32, fontWeight: 700, color: k.accent, fontFamily: "'DM Mono',monospace", letterSpacing: '-1.5px', lineHeight: 1 }}>
            {fmt(k.value)}
          </div>
                  ...
                  return (
                    <tr key={i.id}
                      ...
                      style={{
                        ...
                        background: overdue ? 'rgba(249,115,22,.03)' : isHovered ? 'var(--surf2)' : 'transparent',
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 5, flexWrap: 'wrap' }}>
                          ...
                          {overdue && (
                            <span style={{
                              ...
                              background: T.redBg2, color: T.red, border: `1px solid rgba(229,72,77,.2)`,

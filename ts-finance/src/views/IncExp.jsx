    ...
    >
      <Tbl headers={["Fecha", "Categoría", "Descripción", "Proveedor", "Monto", "Base", "IVA", "Método", ""]}>
        {filtered.length === 0
          ? <tr><td colSpan={9} style={{ textAlign: "center", padding: "48px", color: T.muted, fontSize: 13 }}>Sin registros</td></tr>
          : filtered.map(e => (
            <tr key={e.id} style={{ borderBottom: `1px solid ${T.border}`, opacity: e.autoGenerated ? 0.92 : 1 }}>
              <td style={{ padding: "11px 12px", fontSize: 12, color: T.muted }}>{e.date}</td>
              <td style={{ padding: "11px 12px" }}>
                <span style={{ fontSize: 11, fontWeight: 700, padding: "3px 8px", borderRadius: 6, background: T.surf2, color: T.text2, border: `1px solid ${T.border}` }}>{e.cat}</span>
              </td>
              <td style={{ padding: "11px 12px" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
                  {e.autoGenerated && (
                    <span title="Generado automáticamente desde Recurrentes" style={{ display: "inline-flex", alignItems: "center", gap: 3, fontSize: 9, fontWeight: 700, padding: "2px 6px", borderRadius: 5, background: "rgba(139,92,246,.15)", color: "#8B5CF6", border: "1px solid rgba(139,92,246,.25)", flexShrink: 0 }}>
                      <RefreshCw size={8} />AUTO
                    </span>
                  )}
                  <span style={{ fontSize: 13, color: T.text }}>{e.desc||e.description}</span>
                </div>
              </td>
              ...
              <td style={{ padding: "11px 12px" }}>
                {e.autoGenerated
                  ? <span title="Gestiona este gasto desde Recurrentes" style={{ fontSize: 10, color: T.muted, padding: "3px 7px", borderRadius: 5, border: `1px solid ${T.border}`, cursor: "default" }}>Recurrente</span>

...
const thCell = {
  ...
  background: '#FAFAF9', position: 'sticky', top: 0, zIndex: 1, height: 44,
}
...
function ChipBtn({ active, onClick, children }) {
  return (
    <button onClick={onClick} style={{
      ...
      border: active ? `1px solid #1A2E1F` : `1px solid ${T.border}`,
...
function KpiStrip({ items }) {
  return (
    <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
      {items.map((k, i) => (
        <div key={i} style={{
          flex: '1 1 160px', background: '#FFFFFF', border: '1px solid #E8E9E3',
              ...
              : filtered.map(i => {
                  ...
                  return (
                    <tr key={i.id}
                      ...
                      style={{
                        borderBottom: `1px solid #E8E9E3`,
                        borderLeft: overdue ? `3px solid ${T.red}` : '3px solid transparent',
                        background: overdue ? 'rgba(249,115,22,.03)' : isHovered ? '#F9F8F7' : 'transparent',

...
export const btnSmall = {
  ...btnPrimary, padding: "6px 13px", fontSize: 12, borderRadius: 8,
}

/* ── KPI color themes ────────────────────────────────────────────────── */

const THEME = {
  green:   { bg: T.greenBg,   accent: T.green,   light: "#DCFCE7", dot: T.green  },
  red:     { bg: T.redBg,     accent: T.red,     light: "#FEE2E2", dot: T.red    },
  amber:   { bg: T.amberBg,   accent: T.amber,   light: "#FEF3C7", dot: T.amber  },
  blue:    { bg: T.blueBg,    accent: T.blue,    light: "#DBEAFE", dot: T.blue   },
  violet:  { bg: T.violetBg,  accent: T.violet,  light: "#EDE9FE", dot: T.violet },
  indigo:  { bg: T.accentBg,  accent: T.accent,  light: "#E0E7FF", dot: T.accent },
  default: { bg: T.surf2, accent: T.muted, light: T.surf3, dot: T.muted },
}

function getTheme(color) {
  if (!color) return THEME.default
  const c = color.toString()
  if (c.includes("16A34A") || c.includes("059669") || c.includes("44B26B")) return THEME.green
  ...
  if (c.includes("8B5CF6") || c.includes("7C3AED") || c.includes("5A3EE7")) return THEME.violet
  if (c.includes("6366F1") || c.includes("4F46E5")) return THEME.indigo
...
export function KPI({ title, value, sub, icon: Icon, color, badge, spark, trend }) {
  ...
  return (
    <div style={{ ...card, padding: "20px 22px", display: "flex", flexDirection: "column" }}>
      ...
      {/* Value */}
      <div style={{ fontSize: 28, fontWeight: 700, color: T.text, letterSpacing: "-1px", fontFamily: "'DM Mono',monospace", lineHeight: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{value}</div>
...
export function Pill({ s }) {
  ...
  return (
    <span style={{ fontSize: 11, fontWeight: 600, padding: "3px 9px", borderRadius: 20, background: st.bg, color: st.c, whiteSpace: "nowrap", display: "inline-block" }}>{s}</span>
  )
}
...
export function StatusBar({ items }) {
  return (
    <div style={{ display: "flex", background: T.surf, border: `1px solid ${T.border}`, borderRadius: 16, overflow: "hidden", boxShadow: "0 1px 3px rgba(15,23,42,.06)" }}>
      {items.map(({ label, value, color, bg }, i) => (
        <div key={label} style={{
          ...
          <div style={{ fontSize: 11, fontWeight: 600, color: T.subtle, textTransform: "uppercase", letterSpacing: ".06em", marginBottom: 6 }}>{label}</div>
...
export function AlertBanner({ icon, title, sub, color, bg, border, action, onAction }) {
  return (
    <div style={{ background: bg || "#FFFBEB", border: `1px solid ${border || "#FDE68A"}`, borderRadius: 12, padding: "12px 16px", display: "flex", alignItems: "center", gap: 12 }}>
      ...
      {action && (
        <button onClick={onAction} style={{ fontSize: 12, fontWeight: 600, color: color || T.amber, background: "none", border: "none", cursor: "pointer", padding: "4px 10px", borderRadius: 7, whiteSpace: "nowrap", flexShrink: 0 }}>
          {action}
        </button>
      )}
    </div>
...
export function EmptyState({ icon: Icon, title, sub, action, onAction }) {
  return (
    <div style={{ textAlign: "center", padding: "60px 24px", display: "flex", flexDirection: "column", alignItems: "center", gap: 12 }}>
      {Icon && (
        <div style={{ width: 56, height: 56, borderRadius: 16, background: T.surf2, border: `1px solid ${T.border}`, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 4 }}>
      ...
      {action && (
        <button onClick={onAction} style={{ ...btnPrimary, marginTop: 8 }}>{action}</button>
      )}
    </div>
  )
}
...
export function Tip({ active, payload, label }) {
  ...
  return (
    <div style={{ background: "#0F172A", borderRadius: 12, padding: "10px 16px", boxShadow: "0 12px 32px rgba(15,23,42,.24)" }}>
      ...
      {payload.map((p, i) => (
        <div key={i} style={{ color: "#fff", fontSize: 12, fontWeight: 600, display: "flex", alignItems: "center", gap: 8, marginBottom: i < payload.length - 1 ? 4 : 0, fontFamily: "'DM Mono',monospace" }}>
          ...
          <span style={{ color: "rgba(255,255,255,.45)", minWidth: 62 }}>{p.name}</span>
          {fmtS(p.value)}
        </div>
      ))}
    </div>
...
export const ICON_MAP = {
  ...
  wifi:    { label: "Wi", color: "#3B82F6", bg: "#DBEAFE" },
